...
with open('your_modified_8.csv', 'r', encoding='utf-8') as file:
    reader = csv.DictReader(file)

    for row in reader:
        # Extract ID and title from the row
        recipe_id = row['ID']  # Adjust if the column name is different
        recipe_title = row['title']  # Adjust if the column name is different
        ...
        # Merge the Recipe_Embedding node with its ID and vector properties
        query = """
